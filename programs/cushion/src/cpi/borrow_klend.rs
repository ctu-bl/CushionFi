...
pub fn process_debt_borrow<'info>(
    ctx: &Context<'_, '_, '_, 'info, BorrowAsset<'info>>,
    amount: u64,
) -> Result<()> {
    refresh_reserve_for_borrow(ctx)?;
    refresh_obligation_for_borrow(ctx)?;
    assess_safe_debt_increase(ctx, amount)?;
    ensure_obligation_farm_user_state_for_borrow(ctx)?;
    refresh_obligation_farm_state_for_borrow(ctx)?;
    borrow_liquidity_into_position(ctx, amount)?;
    transfer_borrowed_liquidity_to_user(ctx, amount)
}

fn refresh_reserve_for_borrow<'info>(
    ctx: &Context<'_, '_, '_, 'info, BorrowAsset<'info>>,
) -> Result<()> {
...
fn refresh_obligation_for_borrow<'info>(
    ctx: &Context<'_, '_, '_, 'info, BorrowAsset<'info>>,
) -> Result<()> {
    ...
    if klend_obligation_has_active_reserve(&ctx.accounts.klend_obligation)? {
        let remaining = resolve_borrow_refresh_reserves(ctx)?;
...
fn resolve_borrow_refresh_reserves<'info>(
    ctx: &Context<'_, '_, '_, 'info, BorrowAsset<'info>>,
...
fn ensure_obligation_farm_user_state_for_borrow<'info>(
    ctx: &Context<'_, '_, '_, 'info, BorrowAsset<'info>>,
) -> Result<()> {
...
fn refresh_obligation_farm_state_for_borrow<'info>(
    ctx: &Context<'_, '_, '_, 'info, BorrowAsset<'info>>,
) -> Result<()> {
    let (Some(obligation_farm_user_state), Some(reserve_farm_state)) = (
        ctx.accounts.obligation_farm_user_state.as_ref(),
...
fn borrow_liquidity_into_position<'info>(
    ...
    let active_reserves = resolve_borrow_refresh_reserves(ctx)?;
